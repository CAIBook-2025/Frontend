...
export async function fetchPublicSpaces(accessToken: string | null): Promise<PublicSpace[]> {
  if (!accessToken) {
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/public-spaces`, {
      headers: {
    ...
    return spaces.filter((space: PublicSpace) => space.available === 'AVAILABLE');
  } catch (error) {
...
export async function fetchAllPublicSpaces(accessToken: string | null): Promise<PublicSpace[]> {
  if (!accessToken) {
