...
export const fillGroupName = (name: string) => {
  // Ensure we are on Step 1
  cy.contains('Información General').should('be.visible');

  // Input Name: Break chain to avoid detachment
  cy.get('input[name="name"]').should('be.visible').click();
  // Clear may trigger re-render
  cy.get('input[name="name"]').clear();
  cy.wait(200);

  // Re-query and type
  cy.get('input[name="name"]').should('not.be.disabled').type(name, { force: true, delay: 50 });

  // Re-query element to assert value
  cy.get('input[name="name"]').should('have.value', name);
};

export const fillGroupDescription = (description: string) => {
...
export const fillGroupStep2 = (goal: string) => {
  cy.get('textarea[name="goal"]').should('be.visible').and('not.be.disabled').type(goal, { force: true });
