...
export default function CheckOutClientPage({ reservationId }: { reservationId: number }) {
  ...
  if (isLoading) {
    return <div className="flex h-screen items-center justify-center bg-slate-50"><Loader2 className="h-12 w-12 animate-spin text-blue-600" /></div>;
  }
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50 text-center">
        <div>
          ...
          <Link href="/Reservations" className="mt-4 inline-block text-blue-600 hover:underline">Volver a Mis Reservas</Link>
        </div>
      </div>
    );
  }

  const formattedDate = new Date(reservation.day).toLocaleDateString('es-CL', { weekday: 'long', month: 'long', day: 'numeric' });
  ...
  if (isSuccess) {
    return (
      <div className="flex h-screen w-full flex-col items-center justify-center bg-green-50 text-center animate-fadeIn">
        ...
        <p className="mt-2 text-lg text-slate-600">Has finalizado tu reserva para <strong className="text-gray-800">{reservation.roomName}</strong>.</p>
        <p className="text-slate-500">Módulo {reservation.module} - {formattedDate}</p>
        <Link href="/Reservations" passHref>
          <button className="mt-8 rounded-lg bg-green-600 px-6 py-3 font-semibold text-white transition-colors hover:bg-green-700">Volver a Mis Reservas</button>
        </Link>
      </div>
    );
  }
    ...
    <div className="flex h-screen w-full items-center justify-center bg-slate-50 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl animate-scaleIn">
        ...
        <button
          ...
          {isCheckingOut ? (<><Loader2 className="h-5 w-5 animate-spin" /> Registrando Salida...</>) : ('Confirmar Salida')}
        </button>
        <div className="mt-4 text-center">
          <Link href="/Reservations" className="text-sm text-slate-500 hover:underline">Cancelar y volver</Link>
        </div>
      </div>
    </div>
  );
