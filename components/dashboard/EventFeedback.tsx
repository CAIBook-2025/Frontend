...
export default function EventFeedback({ eventId, isAdmin, isGroupRep, userId, accessToken }: EventFeedbackProps) {
  ...
  const fetchFeedbacks = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/event-feedback/event/${eventId}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
      ...
      const sortedData = feedbacksWithUsers.sort(
        ...
        {displayedFeedbacks.length === 0 ? (
          ...
          displayedFeedbacks.map((feedback) => {
            ...
            return (
              <div
                key={feedback.id}
                className="border border-gray-100 rounded-lg p-4 hover:shadow-sm transition-shadow bg-white"
              >
                ...
                {editingId === feedback.id ? (
                  ...
                  feedback.comment && <p className="text-gray-700 whitespace-pre-wrap mt-2 pl-11">{feedback.comment}</p>
                )}
              </div>
            );
          })
